...
JWT_TOKEN=$(http POST $SERVER_AND_PORT/api/auth Username="$PORTAINER_USERNAME" Password="$PORTAINER_PASSWORD" | jq .jwt | tr -d '"')

echo "Getting list of all stacks and filtering to find $APP_TO_RESTART"
ID=$(http GET $SERVER_AND_PORT/api/stacks "Authorization: Bearer $JWT_TOKEN" | jq ".[] | select(.Name == '$APP_TO_RESTART') | .Id")

echo "Stopping $APP_TO_RESTART"
http POST $SERVER_AND_PORT/api/stacks/$ID/stop "Authorization: Bearer $JWT_TOKEN"

echo "Starting $APP_TO_RESTART"
http POST $SERVER_AND_PORT/api/stacks/$ID/start "Authorization: Bearer $JWT_TOKEN"
